...
- Pi gets a per-repo `sessions/` directory keyed by a `<dirname>-<hash>` slug at `$XDG_DATA_HOME/ramekin/repos/<slug>/sessions/`, mounted at `/root/.pi/agent/sessions`.
- Claude state (`~/.claude/` and `~/.claude.json`) is fully shared across repos. Per-repo isolation of Claude Code's cwd-keyed `projects` map and session-history directory comes from mounting each workspace at `/workspace/<slug>` instead of `/workspace`, so every repo gets a distinct cwd inside the container. Compose's `working_dir` puts the agent there on start.
