...
- Config merges layers, lowest precedence first: binary (the active agent's config allowlist and nothing else — mounted read-only, canonicalized, skip-if-missing; host toolchain config like git and jj is the user layer's job, since its location and its container-compatibility both vary by machine), profile (the active profile's env, mounts, and args), user (every `*.kdl` in `~/.config/ramekin/`, merged as one layer, duplicate keys within the layer are errors), and project (`.ramekin/config.kdl`). A `/dev/null` source masks (removes) a mount inherited from a lower layer. `env` merges per variable; profiles merge by name, last writer takes the whole definition.
...
- Persistence is per-agent, opposite policies: pi is ephemeral-by-default (fresh session dir at `/root/.pi/agent`; allowlisted `auth.json` and the `PI_PERSISTENT_DIRS` caches from `$XDG_DATA_HOME/ramekin/agents/pi/` plus per-repo `sessions/` bound on top; teardown logs discarded writes). Claude is persist-by-default (`~/.claude` + `~/.claude.json` from `$XDG_DATA_HOME/ramekin/agents/`, shared across repos; session-scoped dirs bound over the `CLAUDE_EPHEMERAL` denylist).
